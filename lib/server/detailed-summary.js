...
export async function generateDetailedMeetingSummary({ transcriptPath, metadataPath, force = false }) {
  if (!process.env.OPENAI_API_KEY) {
    throw new Error("Falta OPENAI_API_KEY.");
  }

  const absoluteTranscriptPath = path.resolve(process.cwd(), transcriptPath.replaceAll("/", path.sep));

  // Resolve metadata path: same dir as transcript, same name but .json
  const absoluteMetadataPath = metadataPath
    ? path.resolve(process.cwd(), metadataPath.replaceAll("/", path.sep))
    : absoluteTranscriptPath.replace(/\.txt$/, ".json");

  // Return cached summary if it exists and force=false
  if (!force) {
    try {
      const existing = JSON.parse(await fs.promises.readFile(absoluteMetadataPath, "utf8"));
      if (existing?.detailedSummary?.overview) {
        return existing.detailedSummary;
      }
    } catch {
  ...
  const transcriptText = await fs.promises.readFile(absoluteTranscriptPath, "utf8");
  ...
  try {
    let metadata = {};
    try {
      metadata = JSON.parse(await fs.promises.readFile(absoluteMetadataPath, "utf8"));
    } catch {
      // file may not exist yet
    }
    await fs.promises.writeFile(
      absoluteMetadataPath,
      JSON.stringify({ ...metadata, detailedSummary: result }, null, 2),
      "utf8"
    );
  } catch {
